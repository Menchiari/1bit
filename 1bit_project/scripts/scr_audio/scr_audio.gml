...
function scr_audio_weapon_swing(_frame,_power){
	if image_index==_frame
	{
		...
		audio_play_sound_at(_sound,x,y,y,200,350,1,false,9,_power*global.audio_weapons*_mix,0,random_range(-.1,.1)+clamp(1/(weapon.weight/10),.4,1.8));
	}
}
...
function scr_audio_weapon_equip(_frame,_power){
	if image_index==_frame
	{
		...
		audio_play_sound_at(_sound,x,y,y,200,350,1,false,9,_power*global.audio_hits*_mix,0,random_range(.8,1.2));
	}
}
...
function scr_audio_armor_hit(_frame,_power){
	if image_index==_frame
	{
		...
		audio_play_sound_at(_sound,x,y,y,200,350,1,false,2,_power*global.audio_weapons*_mix,0,random_range(-.1,.1)+clamp(1/(armor.weight/10),.6,1.4)*_pitch);
	}
}
...
function scr_audio_hurt(_frame,_power){
	if image_index==_frame
	{
		...
		audio_play_sound_at(_sound,x,y,y,200,350,1,false,9,_power*global.audio_voices*_mix,0,random_range(.8,1.2));
	}
}
...
function scr_audio_roll(_frame,_power){
	...
	{
		show_debug_message(string(self.name)+" screams");
		audio_play_sound_at(_sound,x,y,y,200,350,1,false,9,_power*global.audio_footsteps*_mix,0,random_range(-.1,.1)+clamp(1/(weapon.weight/10),.4,1.8));
	}
}