		...
		switch ai_state
		{
			...
			#region FOLLOW
				case ai_states.follow:
					...
					{
						if point_distance(x,y,ai_follow_target.x,ai_follow_target.y)>ai_search_range
						//&& !collision_line(x,y,ai_follow_target.x,ai_follow_target.y,obj_collider,true,true)
			...
			#region ALERT
				case ai_states.alert:
					...
					else if irandom_range(0,100)>=99.5/*100-ai_responsiveness*/ {ai_state=ai_state_original;}
				break;
			#endregion
			#region CHASE
			...
			#region SEARCH
				case ai_states.search:
					...
					if chance<=1 {ai_state=ai_state_original;}
					scr_ai_target_check();
					if instance_exists(ai_target)
					{
			...
			#region FLEE
				case ai_states.flee:
					...
					var distr=20
					state=states.run;
					if point_distance(x,y,dest_x,dest_y)<=distr || collision_line(x,y,dest_x,dest_y,obj_collider,true,true)
					{
						dest_x=x+random_range(run_distance_min*4,-run_distance_min*4);
						dest_y=y+random_range(run_distance_min*4,-run_distance_min*4);
					}
					var chancer=irandom_range(0,100);
					if chancer<.5 ai_state=ai_state_original;
				break;
			#endregion
