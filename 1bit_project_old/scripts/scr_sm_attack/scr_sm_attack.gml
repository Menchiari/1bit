function scr_sm_attack(){
	...
	{
		var move_speed=atk1_sp*(spd*defence_speed);
		if weapon.bullets>0 {move_towards_point(x+x-dest_x,y+y-dest_y,move_speed/2);}
		else {move_towards_point(dest_x,dest_y,move_speed);}
	}
	if speed>0 {speed-=atk1_sp_decrease*((spd*defence_speed)/2);}
	else if speed<0 {speed+=atk1_sp_decrease*((spd*defence_speed)/2);}
